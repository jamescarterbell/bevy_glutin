...
pub use glutin_config::*;
pub use glutin_windows::*;
...
#[derive(Default)]
pub struct GlutinPlugin;

impl Plugin for GlutinPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app
            // TODO: It would be great to provide a raw glutin WindowEvent here, but the lifetime on it is
            // stopping us. there are plans to remove the lifetime: https://github.com/rust-windowing/glutin/pull/1456
            // .add_event::<glutin::event::WindowEvent>()
            .init_resource::<GlutinWindows>()
            .init_resource::<GlutinContexts>()
...
pub fn glutin_runner(mut app: App) {
    ...
    log::debug!("Entering glutin event loop");
        ...
        *control_flow = if cfg!(feature = "metal-auto-capture") {
        ...
        match event {
            event::Event::WindowEvent {
                ...
                window_id: glutin_window_id,
                ..
            } => {
                let glutin_windows = app.resources.get_mut::<GlutinWindows>().unwrap();
                let mut windows = app.resources.get_mut::<Windows>().unwrap();
                let window_id = glutin_windows.get_window_id(glutin_window_id).unwrap();
            ...
            event::Event::WindowEvent {
                ...
                WindowEvent::CursorMoved { position, .. } => {
                    ...
                    let glutin_windows = app.resources.get_mut::<GlutinWindows>().unwrap();
                    let window_id = glutin_windows.get_window_id(glutin_window_id).unwrap();
                    let window = glutin_windows.get_window(window_id).unwrap();
...
fn handle_create_window_events(
    ...
    let mut glutin_windows = resources.get_mut::<GlutinWindows>().unwrap();
    ...
    for create_window_event in create_window_event_reader.iter(&create_window_events) {
        ...
        glutin_windows.create_window(event_loop, &window, resources.get_mut::<GlutinContexts>().unwrap().deref_mut());
